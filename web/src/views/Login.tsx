    ...
    return (
      <>
        <div className="login">
          <h2>
            <a
              href={`https://unicorn.gathering.org/oauth/authorize/?client_id=${process.env.REACT_APP_CLIENT_ID}&response_type=code&redirect_uri=${window.location.origin}/login`}
            >
              Log in
            </a>
          </h2>
  ...
  if (fetchStatus === FETCH_STATUS.REJECTED) {
    return (
      <div className="login">
        <h1>Failed.</h1>
        <h3>
          {fetchResult?.message ? fetchResult.message : fetchResult.error}
        </h3>
      </div>
    );
  }
