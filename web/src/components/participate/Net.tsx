...
export const Net = () => {
  const [netParticipationData, setNetParticipationData] =
    useState<NetParticipationData[]>();
  const [timeslot, setTimeslot] = useState<Record<string, any>>({});
  ...
  const fetchParticipationData = useCallback(() => {
    ...
    httpGet<NetParticipationData[]>(
      ...
      .catch((_err) => {
        setNetParticipationData(undefined);
        setFetchStatus(FETCH_STATUS.REJECTED);
      });
  }, [user]);
  ...
  if (fetchStatus === FETCH_STATUS.REJECTED) {
    if (hasSignedUp) {
      ...
      return (
        <>
          <p>
            You are not signed up for the network track. You can sign up{' '}
          ...
          {timeslot?.credentials !== undefined ? (
            ...
            <>
              ...
              <p className="notAssigned">
                Feel free to hang out in Discord in <strong>#tech</strong> or
                the <strong>Tech NOC</strong> voice channel while you wait
              </p>
            </>
          )}
        </section>
